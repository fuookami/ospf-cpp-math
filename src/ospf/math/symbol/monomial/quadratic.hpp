...
namespace std
{
        ...
        : public formatter<string_view, char>
    {
        ...
        inline decltype(auto) format(const ospf::QuadraticMonomialCell<T, PSym, ESym>& cell, FormatContext& fc) const
        {
            ...
            const auto display_name1 = std::visit([](const auto& sym)
                {
                    return sym->display_name();
                }, cell.symbol1());
            if (cell.symbol2().has_value())
            {
                const auto display_name2 = std::visit([](const auto& sym)
                    {
                        return sym->display_name();
                    }, *cell.symbol2());
                return _formatter.format(std::format("{} * {}", display_name1, display_name2), fc);
            }
            else
            {
        ...
        : public formatter<wstring_view, ospf::wchar>
    {
        ...
        inline decltype(auto) format(const ospf::QuadraticMonomialCell<T, PSym, ESym>& cell, FormatContext& fc) const
        {
            ...
            const auto display_name1 = boost::locale::conv::to_utf<ospf::wchar>(string{ std::visit([](const auto& sym)
                {
                    return sym->display_name();
                }, cell.symbol1()) }, locale{});
            if (cell.symbol2().has_value())
            {
                const auto display_name2 = boost::locale::conv::to_utf<ospf::wchar>(string{ std::visit([](const auto& sym)
                    {
                        return sym->display_name();
                    }, *cell.symbol2()) }, locale{});
                return _formatter.format(std::format("L{} * {}", display_name1, display_name2), fc);
            }
            else
            {
                return _formatter.format(display_name1, fc);
